...
然而，這些參數的靜態特性導致了一個廣為人知的問題：在中度至重度負載下，高優先權 AC 壟斷通道存取，使低優先權 AC 遭受嚴重的*飢餓*現象，表現為接近零的吞吐量和過高的封包遺失率。Ugwu 等人 [2] 透過 MATLAB 模擬證實 AIFSN 對此差異的影響最為顯著。Mammeri 等人 [3] 提出基於動態多通道存取的飢餓迴避方法（SDMA），但該方法依賴 802.11ac 的通道綁定，不適用於單通道場景。

### 1.2 Wi-Fi 6/7 與飢餓問題的空白

EDCA 在 Wi-Fi 6（802.11ax）[10] 和 Wi-Fi 7（802.11be）[5] 中仍然是基礎的通道存取機制。近年來相當大比例的研究聚焦於 OBSS/跨 BSS 干擾管理（BSS 著色、OBSS-PD、空間重用）[4] 以及 MLO 下的多鏈路 EDCA 最佳化。這些工作處理的是跨 BSS 或跨鏈路的系統層級資源分配，而針對單一 BSS 內 AC 間飢餓的輕量級 MAC 層機制，相對而言仍較少被探討。

### 1.3 機器學習驅動的 EDCA 最佳化

近年深度強化學習（DRL）方法展現出可觀的成果：Zuo 等人 [6] 提出 PDCF-DRL，在 20–120 個站台的場景下，正規化吞吐量維持在 76% 以上、碰撞率控制在 18% 以下，相比標準 EDCA 的碰撞率 64%–85%、吞吐量僅 13%–32%。Du 等人 [7] 結合聯邦學習與 DDPG，降低 MAC 延遲達 45.9%。Li 等人 [8] 提出 ReinWiFi 框架用於應用層 QoS 最佳化。然而，DRL 方法需要大量運算資源和長時間的訓練收斂，而應用層方案繞過 MAC 層，未能解決根本原因。
...
本專案提出佇列感知動態 EDCA（QAD-EDCA），一種輕量級的規則引擎演算法，填補靜態 EDCA 與高運算成本 DRL 方案之間的空白。QAD-EDCA 的定位**明確不是**在原始吞吐量最佳化上超越 DRL，而是為 DRL 不可行或不必要的場景提供一個實際可部署的替代方案。

**與 DRL 方案的比較**：

| 面向 | DRL 方案（如 PDCF-DRL [6]） | QAD-EDCA（本方案） |
|------|---------------------------|-------------------|
| 需要訓練 | 是（數千個 episode） | **否**（零訓練部署） |
| 首次有效動作時間 | 分鐘至小時 | **100 ms**（第一個信標週期） |
| 運算複雜度 | O(DRL 推論) / 週期 | **O(1)** / 週期 |
| 環境適應性 | 需重新訓練 | 即時適應 |
| 吞吐量最佳化 | 較高 | 中等 |
| 硬體需求 | 偏好 GPU/NPU | 任何 AP 硬體 |

DRL 方案如 PDCF-DRL [6] 透過學習最佳競爭視窗策略，能達到更優的吞吐量最佳化。然而，在訓練階段——可能需要數千個探索 episode——網路效能可能因隨機探索而比標準 EDCA 更差。QAD-EDCA 從第一個監控週期就能產生有效的調整，不需訓練、不需特殊硬體。

**預期貢獻**：

1. **形式化的飢餓偵測準則**：現有文獻以定性方式描述飢餓（「吞吐量趨零」[3]、「封包遺失率高」[2]）。本專案首次提出可量化、可直接實作的偵測公式，結合佇列佔用率與封包遺失率，並正式論證雙指標 OR 邏輯的設計理由。

2. **三維度閉環調整系統**：現有自適應 EDCA 方案通常僅調整 CW。QAD-EDCA 同時調整 AIFSN、CWmin、TXOP——三個作用於通道存取不同階段的參數——搭配有界的調整範圍、指數恢復機制與 QoS 安全閥，形成完整的閉環控制系統。

3. **填補靜態 EDCA 與 DRL 之間的空白**：QAD-EDCA 佔據了一個先前未被探索的中間地帶：它是自適應的（不同於靜態 EDCA），但不需要訓練（不同於 DRL）。這使其適合資源受限的 AP 和延遲敏感的部署場景。

4. **與 DRL 的互補性（暖機能力）**：QAD-EDCA 可作為 DRL 訓練期間的過渡機制——在 DRL 代理於背景訓練的同時提供即時的飢餓緩解，待 DRL 策略收斂後再交接控制權。

**預期限制**：

- QAD-EDCA 使用固定的閾值和縮放因子，可能無法在所有網路條件下達到最優。DRL 能發現人工規則難以設計的參數組合。
- 在流量快速波動的情況下，DRL 的連續策略最佳化可能比 QAD-EDCA 的閾值觸發式調整更精準。
...
3. **PDCF-DRL（Zuo 2025）**：基於深度強化學習的競爭視窗退避方案 [6]。與 PDCF-DRL 的比較基於其論文發表的數據（在可比較的場景設定下），因為重現 DRL 訓練環境超出本專案範圍。DRL 結果作為參考點，用以呈現效能與複雜度之間的權衡。
4. **QAD-EDCA**（本方案）：佇列感知動態調適方案。

主要比較對象為標準 EDCA 和調優靜態 EDCA，兩者在相同條件下模擬。PDCF-DRL 作為吞吐量最佳化的參考上界，而非直接的正面對決基準。
